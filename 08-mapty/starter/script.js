...
class Workout {
  ...
  constructor(coords, distance, duration) {
    this.coords = coords; // store coordinates as an array of lat and long
    this.distance = distance; // store distance in km
    this.duration = duration; // store duration in minutes
  }

  _setDescription() {
    //naka underscore coz it's a private/internal method and can't be used by other classes
    const months = [
      'January',
      'February',
      'March',
...
class Running extends Workout {
  ...
  constructor(coords, distance, duration, cadence) {
    super(coords, distance, duration); //parent class constructor
...
//leaftlet to use interactive maps
class App {
  #map;
  #mapZoomLevel = 13;
  #mapEvent;
  ...
  _loadMap(position) {
    ...
    //creates coordinates array + VVV IMPORTANT
    const coords = [latitude, longitude];

    //pinpoint ng location
    this.#map = L.map('map').setView(coords, this.#mapZoomLevel);

    //add open street map
    L.tileLayer('https://{s}.tile.openstreetmap.fr/hot/{z}/{x}/{y}.png', {
      attribution:
        '&copy; <a href="https://www.opwnstreetmap.org/copyright">OpenStreetMap</a> contributors',
    }).addTo(this.#map);
